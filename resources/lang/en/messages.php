<?php

return [

    /* =========================================================
       HEADER
    ========================================================= */

    'home' => 'Home',
    'products' => 'Products',
    'about' => 'About',
    'favorites' => 'Favorites',
    'search_placeholder' => 'Search products...',


    /* =========================================================
       FOOTER
    ========================================================= */

    'quality' => 'QUALITY',
    'quality_text' => 'quality products',

    'warranty' => 'WARRANTY',
    'warranty_text' => '2 years warranty',

    'free_shipping' => 'FREE SHIPPING',
    'free_shipping_text' => 'up to R$ 150.00',

    'category' => 'Category',
    'service' => 'Support',
    'payment_methods' => 'Payment methods',

    'accessories' => 'Accessories',
    'strings' => 'Strings',
    'amplifiers' => 'Amplifiers',
    'pedals' => 'Pedals/Multi-effects',
    'keys' => 'Keys',
    'percussion' => 'Percussion',
    'wind' => 'Wind',
    'audio_tech' => 'Audio and technology',

    'credit_card' => 'Credit card',
    'debit_card' => 'Debit card',
    'pix' => 'Pix',

    'rights' => '2026. All rights reserved',


    /* =========================================================
       AUTHENTICATION - LOGIN / REGISTER / PASSWORD
    ========================================================= */

    'create_account' => 'Create account',
    'email' => 'E-mail',
    'username' => 'Username',
    'password' => 'Password',
    'confirm_password' => 'Confirm your password',

    'login' => 'Login',
    'forgot_password' => 'forgot your password?',
    'login_google' => 'Login with Google',

    'dont_have_account' => "Don't have an account?",
    'register_here' => 'Sign up here!',

    'or' => 'or',

    'forgot_password_title' => 'Recover password',
    'new_password_title' => 'New password',
    'new_password' => 'New password',
    'send' => 'Send',
    'back' => 'Cancel',


    /* =========================================================
       HOME
    ========================================================= */

    'hero_title' => 'Discover Your<br>New Sound',

    'hero_description' =>
        'Each instrument carries a unique sound. Find the one that will tell your story.',

    'buy' => 'BUY',

    'best_sellers' => 'Best sellers',

    'best_sellers_subtitle' =>
        'discover Instrumental Store best-selling products',

    'add' => 'Add',
    'details' => 'Details',
    'other_products' => 'Other products',
    'in_stock' => ':count in stock',

    'home_products_backend_message' => 'Home products will be loaded from the database.',


    /* =========================================================
       ABOUT PAGE
    ========================================================= */

    'about_title' => 'About Instrumental Store',

    'about_text_1' =>
        'Instrumental Store was founded in 1998 with a simple dream: to democratize access to quality musical instruments in Brazil. Created by passionate musicians, our store started as a small establishment in São Paulo and grew to become a national reference.',

    'about_text_2' =>
        'Throughout more than 25 years of history, we have maintained our commitment to excellence, offering products from the best global brands and specialized service that only those who truly live and breathe music can provide.',

    'about_text_3' =>
        'Today, we serve thousands of musicians across the country, from beginners buying their first instrument to professionals seeking high-performance equipment. Our mission remains the same: to inspire and equip musicians so they can turn their passions into reality.',

    'timeline_title' => 'Our journey',

    'timeline_1968_title' => 'Foundation',
    'timeline_1968_text' => 'The beginning of our story, driven by a passion for musical instruments.',

    'timeline_2005_title' => 'Expansion',
    'timeline_2005_text' => 'Catalog growth and service expansion for new audiences.',

    'timeline_2020_title' => 'E-commerce',
    'timeline_2020_text' => 'Entry into the digital environment to make online shopping easier.',

    'timeline_2025_title' => 'Present',
    'timeline_2025_text' => 'A modern store focused on quality, technology and customer experience.',

    'value_passion_title' => 'Passion for music',
    'value_passion_text' => 'Music inspires everything we do.',

    'value_quality_title' => 'Quality',
    'value_quality_text' => 'We select reliable products for our customers.',

    'value_service_title' => 'Service',
    'value_service_text' => 'We value a simple, clear and secure experience.',

    'objective_title' => 'Our goal',

    'objective_text' =>
        'Our goal is clear: to be the best musical instrument store in Brazil, offering not only quality products, but a complete experience that inspires musicians of all levels to reach their full potential. We want to be by your side at every stage of your musical journey, from the first chord to professional performance.',


    /* =========================================================
       PRODUCTS PAGE
    ========================================================= */

    'products_subtitle' => 'Find the perfect instrument for your sound',
    'all' => 'All',
    'filter' => 'Filter',
    'showing_products' => 'Showing results',
    'all_items' => 'All items',
    'showing' => 'Showing',
    'of' => 'of',
    'results' => 'results',
    'no_products' => 'No products found',
    'next' => 'Next',

    'products_backend_message' => 'Products will be loaded from the database.',


    /* =========================================================
       PRODUCT DETAIL PAGE
    ========================================================= */

    'product_detail_description' =>
        'The perfect guitar for those looking for quality, comfort and a remarkable sound. Ideal for studying, performing and recording.',

    'similar_products' => 'Similar products',


    /* =========================================================
       CLIENT AREA - SIDEBAR
    ========================================================= */

    'client_area' => 'Client area',
    'client_greeting' => 'Hello, customer',
    'client_photo' => 'Customer photo',

    'personal_data' => 'Personal data',
    'addresses' => 'Addresses',
    'orders' => 'Orders',
    'cards' => 'Cards',
    'wishlist' => 'Wishlist',
    'configuration' => 'Settings',

    'logout' => 'Logout',


    /* =========================================================
       CLIENT AREA - PERSONAL DATA
    ========================================================= */

    'full_name' => 'Full Name',
    'birth_date' => 'Birth date',
    'cpf' => 'CPF',
    'mobile' => 'Mobile',
    'gender' => 'Gender',
    'zip_code' => 'ZIP code',
    'street' => 'Street',
    'number' => 'Number',
    'neighborhood' => 'Neighborhood',
    'city' => 'City',
    'state' => 'State',
    'country' => 'Country',

    'select_option' => 'Select',
    'female' => 'Female',
    'male' => 'Male',
    'other' => 'Other',
    'prefer_not_to_say' => 'Prefer not to say',


    /* =========================================================
       CLIENT AREA - ADDRESSES
    ========================================================= */

    'address' => 'Address',
    'register_new_address' => 'Register new address',
    'edit_address' => 'Edit address',
    'complement' => 'Complement',

    'no_addresses' => 'No addresses registered.',
    'addresses_backend_message' => 'Addresses will be loaded from the database.',


    /* =========================================================
       GENERAL ACTIONS
    ========================================================= */

    'edit' => 'Edit',
    'save' => 'Save',
    'delete' => 'Delete',

    /* =========================================================
    ORDERS
    ========================================================= */

    'my_orders' => 'Purchases',
    'order_confirmed' => 'Order confirmed',
    'order_preparing' => 'Order preparing',
    'order_shipping' => 'Order on the way',
    'order_delivered' => 'Order delivered',
    'order_in_progress' => 'Track',
    'order_finished' => 'Finished',
    'order_canceled' => 'Purchase canceled',
    'cancel_order' => 'Cancel order',
    'view' => 'View',

    'no_orders' => 'No orders found.',
    'orders_backend_message' => 'Orders will be loaded from the database.',

    'order_details' => 'Order details',
    'order_details_backend_message' => 'Order details will be loaded from the database.',
    'purchase_date' => 'Purchase date',
    'back_to_orders' => 'Back',

    /* =========================================================
       CARD
    ========================================================= */

    'card' => 'Card',
    'card_nickname' => 'Card nickname',
    'card_type' => 'Card type',
    'card_holder_name' => 'Cardholder name',
    'card_number' => 'Card number',
    'card_ending' => 'ending',
    'expiration_date' => 'Expiration date',
    'security_code' => 'Security code',
    'register_new_card' => 'Register new card',
    'back_to_cards' => 'Back',
    'no_cards' => 'No cards registered.',
    'cards_backend_message' => 'Cards will be loaded from the database.',
    'edit_card' => 'Edit card',

    /* =========================================================
       WISHLIST
    ========================================================= */
  
    'wishlist_title' => 'Wishlist',
    'view_product' => 'View',
    'wishlist_title' => 'Wishlist',
    'view_product' => 'View',
    'no_wishlist_items' => 'No favorite products.',
    'wishlist_backend_message' => 'Favorite products will be loaded from the database.',

    /* =========================================================
       SETTINGS
    ========================================================= */
    'account_settings' => 'Account settings',
    'personal_data_backend_message' => 'Personal data will be loaded from the database.',
    'account_settings' => 'Account settings',
    'account' => 'Account',
    'change_photo' => 'Change photo',
    'country_code' => 'Country code',
'country_flag' => 'Country flag',

   /* =========================================================
       ADMIN PANEL (SIDEBAR & HEADER)
    ========================================================= */
    'admin_panel' => 'ADMIN PANEL',
    'admin_home' => 'HOME',
    'dashboard' => 'DASHBOARD',
    'admin_products' => 'PRODUCTS',
    'inventory' => 'INVENTORY',
    'admin_orders' => 'ORDERS',
    'payments' => 'PAYMENTS',
    'returns' => 'RETURNS',
    'clients' => 'CLIENTS',
    'team' => 'TEAM',
    'logout' => 'Log Out',

   /* =========================================================
      PAINEL ADMIN
   ========================================================= */

   'login_page_title' => 'Team Login',
   'email_column' => 'Email',
   'password' => 'Password',
   'login_btn' => 'Sign In',
   'guitar_image_alt' => 'Instrumental Store Guitar',
   'total_sales' => 'Total Sales',
   'last_7_days' => 'Last 7 days',
   'details' => 'Details',
   'this_week_report' => 'This Week\'s Report',
   'this_week' => 'This week',
   'last_week' => 'Last week',
   'total_products' => 'Total Products',
   'in_stock' => 'In Stock',
   'out_of_stock' => 'Out of Stock',
   'revenue' => 'Revenue',
   'total_orders' => 'Total Orders',
   'all' => 'All',
   'pending' => 'Pending',
   'canceled' => 'Canceled',
   'search_placeholder' => 'search',
   'no' => 'No.',
   'order_id' => 'Order ID',
   'date_column' => 'Date',
   'price' => 'Price',
   'payment' => 'Payment',
   'status' => 'Status',
   'paid' => 'Paid',
   'not_paid' => 'Not Paid',
   'status_pendente' => 'Pending',
   'status_pago' => 'Paid',
   'status_enviado' => 'Shipped',
   'status_entregue' => 'Delivered',
   'status_cancelado' => 'Canceled',
   'products_upper' => 'PRODUCTS',
   'add_product' => 'Add product',
   'cat_accessories' => 'Accessories',
   'cat_strings' => 'Strings',
   'cat_amplifiers' => 'Amplifiers',
   'cat_pedals' => 'Pedals & Effects',
   'cat_percussion' => 'Percussion',
   'cat_audio' => 'Audio & Tech',
   'cat_wind' => 'Wind Instruments',
   'cat_keys' => 'Keys',
   'all_products_count' => 'All Products (:total)',
   'featured_products' => 'Featured Products',
   'on_sale' => 'On Sale',
   'id_column' => 'ID',
   'products_column' => 'Products',
   'created_at_column' => 'Created at',
   'order_column' => 'Order',
   'action_column' => 'Action',
   'no_products_found' => 'No products found matching these filters.',
   'edit_product_upper' => 'EDIT PRODUCT',
   'add_product_upper' => 'ADD PRODUCT',
   'edit_product' => 'Edit product',
   'add_new_product' => 'Add new product',
   'basic_details' => 'Basic details',
   'product_name' => 'Product Name',
   'placeholder_product_name' => 'Ex: Stratocaster Guitar',
   'product_brand' => 'Product Brand',
   'placeholder_product_brand' => 'Ex: Fender',
   'product_description' => 'Product Description',
   'placeholder_product_description' => 'Describe the product details...',
   'pricing' => 'Pricing',
   'product_price' => 'Product price',
   'product_discount' => 'Product discount',
   'optional' => 'Optional',
   'sold_status' => 'Sold',
   'expiration_start' => 'Expiration Start',
   'expiration_end' => 'Expiration End',
   'inventory' => 'Inventory',
   'stock_quantity' => 'Stock quantity',
   'stock_status' => 'Stock status',
   'on_demand' => 'On demand',
   'media_upload_title' => 'Product Media Upload',
   'media_upload_placeholder' => 'Click or drag the main media here',
   'add_images' => 'Add images',
   'categories' => 'Categories',
   'product_category' => 'Product category',
   'select_product_placeholder' => 'select your product',
   'select_color' => 'Select color',
   'cancel' => 'Cancel',
   'save' => 'Save',
   'new_orders' => 'New orders',
   'completed_orders' => 'Completed orders',
   'canceled_orders' => 'Canceled orders',
   'all_orders_count' => 'All Orders (:total)',
   'pending_plural' => 'Pending',
   'shipped_plural' => 'Shipped',
   'delivered_plural' => 'Delivered',
   'canceled_plural' => 'Canceled',
   'search_orders_placeholder' => 'Search by customer or code',
   'code_column' => 'Code',
   'client_column' => 'Customer',
   'purchase_date_column' => 'Purchase Date',
   'total_column' => 'Total',
   'status_column' => 'Status',
   'view_details_title' => 'View Details',
   'no_orders_found' => 'No orders found matching these filters.',
   'back_to_orders' => 'Back to Orders',
   'order_items' => 'Order Items',
   'deleted_product' => 'Deleted Product',
   'quantity' => 'Quantity',
   'unit_price' => 'Unit Price',
   'order_summary' => 'Order Summary',
   'code_label' => 'Code:',
   'purchase_date_label' => 'Purchase Date:',
   'payment_label' => 'Payment:',
   'total_label' => 'Total',
   'customer_data' => 'Customer Data',
   'name_label' => 'Name:',
   'email_label' => 'Email:',
   'total_revenue' => 'Total revenue',
   'completed_transactions' => 'Completed transactions',
   'pending_transactions' => 'Pending transactions',
   'failed_transactions' => 'Failed transactions',
   'completed_plural' => 'Completed',
   'customer_id_column' => 'Customer ID',
   'name_column' => 'Name',
   'method_column' => 'Method',
   'unknown_customer' => 'Unknown',
   'no_payments_found' => 'No payments found.',
   'payment_status_pendente' => 'Pending',
   'payment_status_aprovado' => 'Completed',
   'payment_status_recusado' => 'Canceled',
   'total_returns' => 'Total returns',
   'new_requests' => 'New requests',
   'under_inspection' => 'Under inspection',
   'refunded' => 'Refunded',
   'reason_column' => 'Reason',
   'no_returns_found' => 'No returns found.',
   'status_solicitado' => 'Requested',
   'status_inspecao' => 'Under Inspection',
   'return_status_solicitado' => 'Requested',
   'return_status_aguardando_envio' => 'Awaiting Shipment',
   'return_status_inspecao' => 'Under Inspection',
   'return_status_reembolsado' => 'Refunded',
   'return_status_recusado' => 'Rejected',
   'back_to_returns' => 'Back to Returns',
   'current_status' => 'Current Status',
   'return_information' => 'Return Information',
   'selected_reason' => 'Selected reason:',
   'tracking_code_label' => 'Tracking Code:',
   'not_posted_yet' => 'Not posted yet',
   'customer_notes' => 'Customer Notes:',
   'no_notes_provided' => 'No additional notes provided.',
   'original_order_items' => 'Original Order Items (#:code)',
   'actions_required' => 'Actions Required',
   'update_return_notice' => 'Update the progress of this return to notify the customer.',
   'btn_authorize_shipping' => 'Authorize Shipping',
   'btn_refuse_request' => 'Refuse Request',
   'btn_confirm_receipt' => 'Confirm Receipt',
   'btn_authorize_refund' => 'Authorize Refund',
   'btn_reject_inspection' => 'Reject Inspection',
   'process_completed' => 'Process Completed',
   'financial_summary' => 'Financial Summary',
   'amount_paid' => 'Amount Paid:',
   'amount_to_refund' => 'Amount to Refund',
   'total_clients' => 'Total customers',
   'new_clients' => 'New customers',
   'last_30_days' => 'Last 30 days',
   'male_audience' => 'Male Audience',
   'female_audience' => 'Female Audience',
   'all_count' => 'All (:total)',
   'search_clients_placeholder' => 'Search by name, email or CPF',
   'cpf_column' => 'CPF',
   'phone_column' => 'Phone',
   'gender_column' => 'Gender',
   'registration_date_column' => 'Registration Date',
   'not_informed' => 'Not informed',
   'gender_prefiro_nao_informar' => 'Prefer not to say',
   'no_clients_found' => 'No customers found.',
   'gender_masculino' => 'Male',
   'gender_feminino' => 'Female',
   'gender_outro' => 'Others',
   'team_and_access' => 'Team and Access Control',
   'new_employee' => 'New Employee',
   'total_team' => 'Total Team',
   'role_admins' => 'Administrators',
   'role_managers' => 'Managers',
   'role_operators' => 'Operators',
   'search_team_placeholder' => 'search name or email',
   'role_column' => 'Role',
   'no_team_found' => 'No team members found.',
   'role_admin' => 'Administrator',
   'role_gerente' => 'Manager',
   'role_operador' => 'Operator',
   'status_active' => 'Active',
   'status_inactive' => 'Inactive',
   'breadcrumb_edit_employee' => 'Employees > Edit',
   'breadcrumb_create_employee' => 'Employees > Register',
   'back_to_team' => 'Back to Team',
   'edit_employee' => 'Edit Employee',
   'create_new_employee' => 'Register New Employee',
   'full_name' => 'Full Name',
   'access_email' => 'Access Email',
   'initial_password' => 'Initial Password',
   'new_password_help' => 'New Password (leave blank to keep current)',
   'access_level' => 'Access Level (Role)',
   'account_status' => 'Account Status',
   'update_profile_photo' => 'Update Profile Photo',
   'profile_photo_optional' => 'Profile Photo (Optional)',
   'save_changes' => 'Save Changes',
   'register_employee_btn' => 'Register Employee',
   'btn_choose_file' => 'Choose file',
   'no_file_chosen' => 'No file chosen',

/* =========================================================
   CART
========================================================= */

'cart' => 'Cart',
'shopping_cart' => 'Shopping cart',
'cart_empty' => 'Your cart is empty.',
'view_cart' => 'View cart',
'product' => 'Product',
'price' => 'Price',
'quantity' => 'Quantity',
'subtotal' => 'Subtotal',
'total' => 'Total',
'remove' => 'Remove',
'finish' => 'Finish',
'checkout' => 'Checkout',
'coupon' => 'Coupon',
'enter_coupon' => 'Enter coupon',
'apply' => 'Apply',
'see_products' => 'See products',
'items_available' => 'items available',

/* =========================================================
   PAYMENT / PURCHASE DATA
========================================================= */

'payment_form' => 'Payment form',
'purchase_data' => 'Purchase data',
'payment_method' => 'Payment method',
'select' => 'Select',
'credit_card' => 'Credit card',
'debit_card' => 'Debit card',
'pix' => 'Pix',
'registered_cards' => 'Registered cards',
'select_registered_card' => 'Select registered card',
'no_registered_cards' => 'No registered cards',
'register_payment_method' => 'Register method',
'registered_address' => 'Registered address',
'select_registered_address' => 'Select registered address',
'no_registered_address' => 'No registered address',
'register_address' => 'Register address',
'no_products_in_cart' => 'No products in cart',
'back' => 'Back',
'purchase_completed' => 'Purchase completed',
'purchase_success_message' => 'Purchase completed successfully',
'back_to_products' => 'Back to products',
'thank_you_purchase' => 'Thank you for shopping with us!',
'pix_payment' => 'Pix payment',
'pay_with_pix' => 'Pay with Pix',
'pix_instruction' => 'Scan the QR Code or copy the Pix code below to complete the payment.',
'pix_code' => 'Pix code',
'copy' => 'Copy',
'pix_copied' => 'Pix code copied!',
'confirm_payment' => 'Confirm payment',
];